...
router.get("/", async (req, res) => {
  const courentDate = new Date().toISOString().slice(0, 19).replace('T', ' ');
    try {
    let leftReservation = await knex('meals').select(knex.raw('DISTINCT meals.id,meals.hostName, meals.title, meals.description AS mealDescrip, meals.location, meals.when, meals.max_reservations, meals.price,meals.image, SUM(DISTINCT reservations.number_of_guests)  AS totalOfGuests, AVG(reviews.stars) AS totalStars ,GROUP_CONCAT(DISTINCT reviews.description) AS revDescrip'))
